...
Codex가 활성화된 경우 Plan 검증을 Codex에 위임하고, plan-verifier는 결과 정규화에 집중합니다.

위임 절차:

1. **Task tool로 `codex-rescue` subagent 호출** -- Plan 검증 위임
2. **위임 프롬프트 구성** -- 아래 항목을 포함하여 전달:
   - 입력 문서 경로:
     - `.claude/plan/plan.md`
     - `.claude/plan/context.md`
     - `.claude/plan/checklist.md`
   - 검증 관점 5가지 (각 관점에 대한 PASS/이슈 판정 요청):
     - 목적 정합성: Plan이 원래 요청의 목적/문제/방법과 일치하는지
     - 완전성: 누락된 파일/import/타입/설정이 있는지
     - 논리적 일관성: Task 간 의존성 충돌, 순서 문제, 상충 변경 여부
     - 실현 가능성: Plan이 참조하는 파일/함수/타입이 실제 존재하는지
     - 스코프 초과: 목적과 무관한 작업이 포함되었는지
3. **Codex 응답 수신 후 plan-verifier가 정규화** -- 아래 책임은 Claude에 남습니다:
   - Codex가 발견한 이슈를 기존 `<output_format>` 섹션의 5개 항목별 PASS/DRIFT/INCOMPLETE/CONFLICT/UNVERIFIED/OVER_SCOPE 판정 형식으로 재구성
   - 최종 판정(APPROVED / REVIEW_NEEDED / REWORK) 결정
   - Critical/Warning 분류표로 정리하여 사용자에게 전달
   - 필요 시 Glob/Grep/Read로 의심 항목 추가 확인하여 근거 보강

> Codex는 분석/제안 단계, plan-verifier는 결과 정규화 및 최종 판정을 담당합니다.

### B. codex_unavailable

Codex가 없거나 인증되지 않은 경우 아래 `<instructions>` 섹션의 기존 검증 로직(코드베이스 탐색을 통한 능동적 검증)을 그대로 진행합니다.
