...
| 파일 | 역할 | 핵심 필드 |
|------|------|---------|
| templates/global/agents/code-writer.md | 코드 구현 전문 Agent | PostToolUse(코드리뷰), Stop(followup), skills preload |
...
| templates/global/agents/git-manager.md | Git 작업 전문 Agent | Stop hook, skills preload |
...
| templates/global/agents/context-manager.md | Context 문서 관리 Agent | skills preload (PostToolUse hook 없음) |
| templates/global/settings.json | 전역 필수 hook만 유지 | UserPromptSubmit, PreToolUse |
| ~~templates/global/hooks/skill-forced-subagent.sh~~ | ~~SubagentStart hook (삭제됨)~~ | ~~Skills 2.0 frontmatter로 대체~~ |
...
5. Skill preload: SubagentStart hook(`skill-forced-subagent.sh`) 삭제 → Agent frontmatter `skills` 필드로 대체 (시작 시 자동 주입)
