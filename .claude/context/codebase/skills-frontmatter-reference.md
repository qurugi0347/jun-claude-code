...
keywords: [frontmatter, SKILL.md, agent, yaml, skills-2.0, reference, hooks, permissionMode, coordinatorMode]
---
...
| 병렬 오케스트레이션 | - | coordinatorMode (experimental) |

## coordinatorMode 필드 (experimental)

Agent frontmatter에 추가된 실험적 필드입니다.

| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `coordinatorMode` | boolean | `false` | `true` → Coordinator Mode에서 worker로 실행 가능 |

활성화 방법: 환경변수 `CLAUDE_CODE_COORDINATOR_MODE=true` 설정 후 Claude Code 실행.
상세 가이드: `templates/global/skills/PromptStructuring/advanced-modes.md` 참조.
