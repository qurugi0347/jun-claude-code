...
keywords: [hooks, worktree, 안정성, 실행환경, gpm, 자동화]
---
...
Claude Code hook은 세션 시작, 도구 실행, 응답 완료 시점에 자동으로 실행되어 GPM 워크플로우를 지원합니다. 그러나 worktree나 프로젝트 하위 디렉토리에서 실행할 경우 상대 경로 기반의 hook 스크립트가 찾아지지 않아 자동화가 중단됩니다. 이 신뢰성 문제를 해결하여 어떤 환경에서도 일관된 자동화 경험을 제공합니다.
...
| 경로 독립적 실행 | git 루트 기반으로 hook 스크립트 경로 해결 | cwd에 무관하게 hook이 항상 동작 |
