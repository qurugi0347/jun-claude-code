...
keywords: [install, multiselect, UX, 파일선택, 카테고리, 상태표시, commands]
---
...
agents/commands/skills/others 카테고리 분류와 new/changed/unchanged 상태 표시로 설치 전 전체 상황을 한눈에 파악할 수 있습니다.

## 핵심 기능

| 기능 | 설명 | 사용자 관점 |
|------|------|------------|
| 카테고리 분류 | 파일을 agents / commands / skills / others 4그룹으로 구분 | 역할별로 묶어 선택 용이 |
...
| global/project 모드 분리 | settings.json의 statusLine과 statusline-command.sh는 global 설치에만 포함 | 개인 환경 설정과 프로젝트 설정 분리 |

## 사용자 흐름

1. `jun-claude-code install` 실행 → 소스 파일 목록 수집
2. agents / commands / skills / others 카테고리로 분류 표시
3. Agents MultiSelect 화면 → 설치할 agent 선택
4. Commands MultiSelect 화면 → 설치할 slash command 선택
5. Skills 2단계 선택:
   - 1단계: Skills MultiSelect 화면 → 설치할 스킬 디렉토리 선택
   - 2단계: 하위 파일 MultiSelect 화면 → 스킬별 구분선으로 그룹화하여 파일 세분화 선택 (하위 파일이 1개인 스킬은 자동 포함)
6. others(hooks, CLAUDE.md 등) → 변경된 항목만 자동 복사
7. settings.json → 기존 설정 보존하며 병합 적용 (global 모드: statusLine 없으면 템플릿 값 적용, project 모드: statusLine 제외)
8. 설치 완료 요약 출력
